...
def _selected_slots(document: dict):
    ...
    excluded from the answer document (issue #125 decision).
    """
...
def build_answer_document(paper) -> dict:
    """Build the ``paper_answer_document.v1`` for a paper from its bank answers.
    ...
    editor-facing provenance, and ``modified: False`` — a fresh paper-local
    snapshot the editor owns from here on. A missing bank answer never fails
    assembly; it yields an empty-content entry.
    """
    ...
    * it is an unverified generated draft the teacher has not reviewed
    ...
    for slot_id, entry in entries.items():
        if entry.get("source") == SOURCE_GENERATED and not entry.get("modified"):
            continue
        text = _content_to_text(entry.get("content") or [])
        if text:
...
def _content_to_text(content: list[dict]) -> str:
    ...
    parts = [item.get("text") or item.get("latex") or "" for item in content]
